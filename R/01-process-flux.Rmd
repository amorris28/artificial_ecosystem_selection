...
selected <- read_csv(paste0(raw_dir, 'selected.csv'))
```

# Overview

This script processes the standard curve data from the GC on each sample date. It then converts area to methane concentration using the standard curves for every sample. Finally, it computes fluxes (change in concentration over time) as a first-order exponential decay function. These are exported into the `r der_dir` directory.
...
This script first imports the values for the standard curves. It then calculates the slope of Methane on Area for each sample date.
