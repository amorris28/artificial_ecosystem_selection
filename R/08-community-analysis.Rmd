...
save(kt, wt, rare_median, s, min_seqs, file = "Output/richness_models.RData")
...
rownames(passage2_df) <- passage2_df$sample
passage2_df <- passage2_df[, -1]
...
```{r passage2-dbrda}
if(file.exists("Output/passage2_rare_dist_bray.rds")) {
  passage2_rare_dist_bray <- 
    readRDS("Output/passage2_rare_dist_bray.rds")
} else {
passage2_rare_dist_bray <- avgdist(passage2_df, dmethod="bray", sample=seq_range[1])
  passage2_rare_dist_bray %>% 
    saveRDS("Output/passage2_rare_dist_bray.rds")
}
...
passage2_meta_dist <-
  ...
  select(sample, passage_treat, passage, treat, ch4) %>% 
  distinct %>% 
  inner_join(passage2_dist_df, by = "sample")


passage2_dbrda_ch4 <- dbrda(passage2_rare_dist_bray ~ rank(ch4), 
                      data = passage2_meta_dist, 
                      na.action = "na.omit",
                      add = TRUE) 

passage2_dbrda_permtest <- 
passage2_dbrda_ch4 %>% 
  permutest(permutations = 9999)
...
passage2_percent_explained <- 100 * eigenvals(passage2_dbrda_ch4) / sum(eigenvals(passage2_dbrda_ch4))

passage2_dbrda_scores <-
  scores(passage2_dbrda_ch4, display = "sites") %>% 
  as_tibble(rownames = "sample")
...
passage2_dbrda_data <-
  ...
  select(sample, passage, treat, ch4) %>% 
  distinct %>% 
  inner_join(passage2_dbrda_scores, by = "sample")
...
```{r passage5-dbrda}
if(file.exists("Output/passage5_rare_dist_bray.rds")) {
  passage5_rare_dist_bray <- 
    readRDS("Output/passage5_rare_dist_bray.rds")
} else {
passage5_rare_dist_bray <- avgdist(passage5_df, dmethod="bray", sample=seq_range[1])
  passage5_rare_dist_bray %>% 
    saveRDS("Output/passage5_rare_dist_bray.rds")
}
...
passage5_meta_dist <-
  ...
  select(sample, passage_treat, passage, treat, ch4) %>% 
  distinct %>% 
  inner_join(passage5_dist_df, by = "sample")


passage5_dbrda_ch4 <- dbrda(passage5_rare_dist_bray ~ rank(ch4), 
                      data = passage5_meta_dist, 
                      na.action = "na.omit",
                      add = TRUE) 

passage5_dbrda_permtest <-
passage5_dbrda_ch4 %>% 
  anova(permutations = 9999) %>% 
  tidy

passage5_percent_explained <- 100 * eigenvals(passage5_dbrda_ch4) / sum(eigenvals(passage5_dbrda_ch4))

passage5_dbrda_scores <-
  scores(passage5_dbrda_ch4, display = "sites") %>% 
  as_tibble(rownames = "sample")
...
passage5_dbrda_data <-
  ...
  select(sample, passage, treat, ch4) %>% 
  distinct %>% 
  inner_join(passage5_dbrda_scores, by = "sample")
...
```{r}
save(passage2_dbrda_permtest, 
     passage5_dbrda_permtest, 
     passage5_percent_explained,
     file = "Output/dbrda.RData")
```
...
ggsave('Manuscript/fig4.jpg', p_log10, width = 5, height = 5) 


## All
# ggsave("shared_taxa.png", p)
ggsave("shared_taxa_log.png", p_log10, width = 5, height = 5)
...
```{r setup-differential-abundance-data-genus}
...
aes_Genus_G5 <-
  aes_G5 %>% 
  select(asv, sample, treat, count, Genus) %>%
  group_by(sample, Genus) %>% 
  mutate(total = sum(count)) %>% 
  ungroup() %>% 
  select(sample, treat, Genus, total) %>% 
  distinct()
...
otu_table <- 
  aes_Genus_G5 %>% 
  filter(Genus %in% prevalent_families) %>% 
...
if(file.exists("Output/ancom_out_genus.rds")) {
  out <- 
    readRDS("Output/ancom_out_genus.rds")
} else {
  # Run Ancom
  out = ancom(phyloseq = aes_ps,  
              p_adj_method = "BH",
              ...
              n_cl = 1)  
  out %>% 
    saveRDS("Output/ancom_out_genus.rds")
}
...
if(file.exists("Output/aldex_x_all_genus.rds")) {
  x.all <- 
    readRDS("Output/aldex_x_all_genus.rds")
} else {
  x.all <- aldex(otu_table, 
                 conds, mc.samples=1000, test="t", effect=TRUE,
                 include.sample.summary=FALSE, denom="all", verbose=FALSE, paired.test=FALSE)
  x.all %>% 
    saveRDS("Output/aldex_x_all_genus.rds")
}
...
# ALDEX2
aldex_taxa <- x.all_effect_1$Genus
...
at_least_one_taxa <-
	...
	group_by(sample) %>%
	mutate(rel_abund = (total / sum(total)) * 100) %>%
	filter(Genus %in% ancom_taxa | Genus %in% aldex_taxa | Genus %in% corncob_taxa) %>%
	mutate(Genus = str_replace(Genus, "(.*)", "*\\1*"),
	       Genus = str_replace(Genus, "\\*(.*)_unclassified\\*",
				    "Unclassified *\\1*"))

at_least_one_iqm <- 
	at_least_one_taxa %>%
	group_by(treat, Genus) %>%
	summarize(median = median(rel_abund), 
		  first_quartile = quart(rel_abund)[1],
		  third_quartile = quart(rel_abund)[2], .groups = "drop")
...
at_least_one_taxa$Genus <- factor(at_least_one_taxa$Genus, levels = sort_order$Genus)
...
all_taxa <-
	...
	group_by(sample) %>%
	mutate(rel_abund = (total / sum(total)) * 100) %>%
  left_join(aldex_effect, by = "Genus") %>% 
	filter(Genus %in% ancom_taxa & Genus %in% aldex_taxa & Genus %in% corncob_taxa) %>% 
  ungroup()
...
all_taxa <-
  all_taxa %>%
	mutate(Genus = str_replace(Genus, "(.*)", "*\\1*"),
	       Genus = str_replace(Genus, "\\*(.*)_unclassified\\*",
				    "Unclassified *\\1*"))
# all_taxa$Genus[all_taxa$Genus == "Unclassified *0319-6G20*"] <- "Uncultured Genus 0319-6G20"
# all_taxa$Genus[all_taxa$Genus == "Unclassified *Chloroplast*"] <- "Chloroplast"
# all_taxa$Genus[all_taxa$Genus == "Unclassified *Armatimonadales*"] <- "Unclassified Armatimonadales"
# all_taxa$Genus[all_taxa$Genus == "Unclassified *Gammaproteobacteria*"] <- "Unclassified Gammaproteobacteria"
# all_taxa$Genus[all_taxa$Genus == "Unclassified *Kapabacteriales*"] <- "Unclassified Kapabacteriales"
...
all_taxa$Genus <- factor(all_taxa$Genus, levels = sort_order$Genus)
all_iqm$Genus <- factor(all_iqm$Genus, levels = sort_order$Genus)
all_taxa <-
  all_taxa %>% 
  mutate(direction = if_else(effect > 0, 0, 1))
all_iqm <-
all_taxa %>% 
  select(Genus, direction) %>% 
  distinct() %>% 
  full_join(all_iqm, by = "Genus")
(p <-
	all_taxa %>%
	ggplot(aes(x = rel_abund, y = Genus, color = treat)) +
	geom_point(position=position_jitterdodge()) +
...
# 	filter(Genus %in% ancom_taxa & Genus %in% aldex_taxa & Genus %in% corncob_taxa) %>% 
#   pivot_longer(G2:G5, names_to = "passage", values_to = "rclr") %>% 
#   select(Genus, treat, rep, passage, rclr) %>% 
#   distinct() %>% 	
#   mutate(Genus = str_replace(Genus, "(.*)", "*\\1*"),
# 	       Genus = str_replace(Genus, "\\*(.*)_unclassified\\*",
# 				    "Unclassified *\\1*"))
# 
# aes_Genus_rclr_plot$Genus[aes_Genus_rclr_plot$Genus == "Unclassified *0319-6G20*"] <- "Uncultured Genus 0319-6G20"
# aes_Genus_rclr_plot$Genus[aes_Genus_rclr_plot$Genus == "Unclassified *Chloroplast*"] <- "Chloroplast"
# aes_Genus_rclr_plot$Genus <- factor(aes_Genus_rclr_plot$Genus, levels = sort_order$Genus)
# ggplot(aes_Genus_rclr_plot, aes(x = rclr, y = Genus, color = treat)) +
#   facet_grid( ~ passage) +
#   geom_point(position = position_jitterdodge())

p_effect <-
	all_taxa %>%
	ggplot(aes(x = effect, y = Genus)) +
	geom_point() +
...
```{r setup-differential-abundance-data-species}
...
aes_Species_G5 <-
  aes_G5 %>% 
  select(asv, sample, treat, count, Species) %>%
  group_by(sample, Species) %>% 
  mutate(total = sum(count)) %>% 
  ungroup() %>% 
  select(sample, treat, Species, total) %>% 
  distinct()
...
otu_table <- 
  aes_Species_G5 %>% 
  filter(Species %in% prevalent_families) %>% 
  select(sample, Species, total) %>% 
  pivot_wider(names_from = sample, values_from = total, values_fill = 0) %>% 
  as.data.frame()
...
# x.all_effect_1 %>% 
#   # inner_join(taxa_table, by = "asv")
#   ggplot(aes(y = reorder(Species, effect), x = effect)) +
#   geom_point()
...
# ALDEX2
aldex_taxa <- x.all_effect_1$Species
...
at_least_one_taxa <-
	...
	group_by(sample) %>%
	mutate(rel_abund = (total / sum(total)) * 100) %>%
	filter(Species %in% ancom_taxa | Species %in% aldex_taxa | Species %in% corncob_taxa) %>%
	mutate(Species = str_replace(Species, "(.*)", "*\\1*"),
	       Species = str_replace(Species, "\\*(.*)_unclassified\\*",
				    "Unclassified *\\1*"))
...
at_least_one_taxa$Species <- factor(at_least_one_taxa$Species, levels = sort_order$Species)
...
all_taxa <-
	...
	group_by(sample) %>%
	mutate(rel_abund = (total / sum(total)) * 100) %>%
  left_join(aldex_effect, by = "Species") %>% 
	filter(Species %in% ancom_taxa & Species %in% aldex_taxa & Species %in% corncob_taxa) %>% 
  ungroup()
...
all_taxa <-
  all_taxa %>%
	mutate(Species = str_replace(Species, "(.*)", "*\\1*"),
	       Species = str_replace(Species, "\\*(.*)_unclassified\\*",
				    "Unclassified *\\1*"))
all_taxa$Species[all_taxa$Species == "Unclassified *0319-6G20*"] <- "Uncultured Species 0319-6G20"
all_taxa$Species[all_taxa$Species == "Unclassified *Chloroplast*"] <- "Chloroplast"
all_taxa$Species[all_taxa$Species == "Unclassified *Armatimonadales*"] <- "Unclassified Armatimonadales"
all_taxa$Species[all_taxa$Species == "Unclassified *Gammaproteobacteria*"] <- "Unclassified Gammaproteobacteria"
all_taxa$Species[all_taxa$Species == "Unclassified *Kapabacteriales*"] <- "Unclassified Kapabacteriales"
...
all_taxa$Species <- factor(all_taxa$Species, levels = sort_order$Species)
all_iqm$Species <- factor(all_iqm$Species, levels = sort_order$Species)
all_taxa <-
  all_taxa %>% 
  mutate(direction = if_else(effect > 0, 0, 1))
all_iqm <-
...
# aes_Species_rclr_plot <-
#   aes_Species_rclr %>% 
# 	filter(Species %in% ancom_taxa & Species %in% aldex_taxa & Species %in% corncob_taxa) %>% 
#   pivot_longer(G2:G5, names_to = "passage", values_to = "rclr") %>% 
#   select(Species, treat, rep, passage, rclr) %>% 
#   distinct() %>% 	
#   mutate(Species = str_replace(Species, "(.*)", "*\\1*"),
# 	       Species = str_replace(Species, "\\*(.*)_unclassified\\*",
# 				    "Unclassified *\\1*"))
# 
# aes_Species_rclr_plot$Species[aes_Species_rclr_plot$Species == "Unclassified *0319-6G20*"] <- "Uncultured Species 0319-6G20"
# aes_Species_rclr_plot$Species[aes_Species_rclr_plot$Species == "Unclassified *Chloroplast*"] <- "Chloroplast"
# aes_Species_rclr_plot$Species <- factor(aes_Species_rclr_plot$Species, levels = sort_order$Species)
# ggplot(aes_Species_rclr_plot, aes(x = rclr, y = Species, color = treat)) +
#   facet_grid( ~ passage) +
#   geom_point(position = position_jitterdodge())

p_effect <-
	all_taxa %>%
	ggplot(aes(x = effect, y = Species)) +
	geom_point() +
