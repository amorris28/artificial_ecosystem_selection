...
This script intakes the flux data from script 01. It then fits a linear model to the flux data over time per treatment and tests a difference of slope between the positive and the neutral treatment. Finally, it 
...
inter_model <- lm(log_ch4 ~ passage * treat, data = fluxes)
...
compare_mods <- anova(inter_model, no_treat_model)
...
The intercept for the Neutral treatment was significantly different from zero at `r round(tidy(inter_model)[[1, 2]], 2)` (SE = `r round(tidy(inter_model)[[1, 3]], 2)`, t = `r round(tidy(inter_model)[[1, 4]], 2)`, p = `r round(tidy(inter_model)[[1, 5]], 2)`). The Positive treatment started at a significantly lower methane oxidation rate than the Neutral treatment with a y-intercept of `r round(tidy(inter_model)[[3, 2]], 2)` relative to Neutral (SE = `r round(tidy(inter_model)[[3, 3]], 2)`, t = `r round(tidy(inter_model)[[3, 4]], 2)`, p = `r round(tidy(inter_model)[[3, 5]], 2)`). There was no change in methane oxidation rate of the Neutral treatment over the five passages (slope = `r round(tidy(inter_model)[[2, 2]], 2)`, SE = `r round(tidy(inter_model)[[2, 3]], 2)`, t = `r round(tidy(inter_model)[[2, 4]], 2)`, p = `r round(tidy(inter_model)[[2, 5]], 2)`). By contrast, there was a significant increase in the Positive treatment of `r round(tidy(inter_model)[[4, 2]], 2)` per passage which is approximately a `r round((pos_slope - 1) * 100, 0)`% increase in methane oxidation rate per passage (SE = `r round(tidy(inter_model)[[4, 3]], 2)`, t = `r round(tidy(inter_model)[[4, 4]], 2)`, p = `r round(tidy(inter_model)[[4, 5]], 2)`).
