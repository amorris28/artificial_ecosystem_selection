...
fluxes %>% 
  group_by(passage) %>% 
  mutate(passage_mean = mean(estimate)) %>% 
  mutate(deviance = estimate - passage_mean)

ggplot(fluxes, aes(x = passage, y = deviance, color = treat)) + 
  geom_point() +
  stat_smooth(method = 'lm')

fit <- lm(deviance ~ treat * passage, data = fluxes)
summary(fit)
```
