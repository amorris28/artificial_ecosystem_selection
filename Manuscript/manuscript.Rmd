...
methane oxidation rate per round over five rounds of selection. In addition, we
...
functions such as production, consumption, and nutrient release
...
describing a more detailed mapping between the microbiome and ecosystem
function has been elusive [@conrad1996; @rocca2015; @graham2016].
...
However, microbiome attributes add an average of only 8% of variance explained
to ecosystem models [@graham2016]. 
...
little is understood about microbial diversity, seems highly unlikely. And for
most functions in most ecosystems there is no correlation between the abundance
of a marker gene and the rate of the corresponding process [@rocca2015]. 
...
among microbiologists [@swenson2000; @blouin2015; @panke-buisse2015; @sanchez2021].
...
could be attributed independently to microbiome variation [@goodnight2000; @blouin2015]. 
By "microbiome variation," we mean aspects of microbial diversity that are
routinely measured by microbial ecologists, such as the relative
abundance of taxonomic groups inferred from amplicon sequence variants. 
In
...
Such an approach has the potential to expand our ability to more
...
In this study, we wanted to evaluate whether variation in the microbiome
contributes to variation in the rate of ecosystem functions and to identify
microbiome attributes that might explain the mapping between microbiome and
ecosystem function. To begin to address this problem, we used artificial
ecosystem selection on soil microbiomes as a model system.  Using this
experimental model, we answered the following questions: Does variation in the
relative abundance of microbial taxa contribute to variation in soil methane
oxidation rate in the lab? How much of the variation in soil methane oxidation
rate can be attributed to the microbiome? What attributes of the microbiome
regulate variation in methane oxidation rate? And do these attributes match our
assumptions about what regulates methane oxidation rate?  We applied artificial
ecosystem selection on soil microbiomes by selecting on the ecosystem-scale
CH~4~ oxidation rate. We estimated how much of the variation in CH~4~ oxidation
rate was associated with variation in the microbiome by calculating the slope
of the regression between donor and recipient communities. To identify which
microorganisms could be markers of CH~4~ oxidation rate, we compared the
composition of the artificially selected microbiomes to a control set of
microbiomes with random selection. We then evaluated whether these markers met
...
ASVs were removed. Amplicon sequence variants that were assigned chloroplast or mitochondria taxonomy were removed prior to analysis.
...
We estimated the proportion of variance in CH~4~ oxidation rate due to variation in
the microbiome as the
regression of recipient on donor [@falconer1996]. The mid-donor was the
mean for all three selected jars and the mid-recipient was the mean methane oxidation 
rate for the twelve jars produced by those three donors. First, we tested if there was an effect of treatment by comparing nested models with and without treatment using an F-test with the 'anova' function. We then fit the full model to estimate the
slope of the relationship between recipient and donor for the Positive and Neutral treatments.
...
treatment relative to the Neutral selection treatment. This suggests that 
variation in the microbiome can generate variation in ecosystem
functions independent of the environment.

We wanted to estimate how much of the variation in ecosystem function can be
attributed to variation in the microbiome. This problem is analogous to the
problem in quantitative genetics of estimating how much of the variation in an
organismal trait is due variation in genotype, i.e., heritability. Organismal
biologists often estimate the percent heritability of a trait as the slope of the
regression of offspring phenotype
on parental phenotype [@falconer1996]. A correlation between the trait values 
of parents and offspring indicates that the trait is at least partially 
genetically determined. Using artificial
ecosystem selection we can estimate the variation in an ecosystem trait due to
variation in the microbiome by comparing the trait values of the "offspring" or
recipient jars in one generation to the trait value of the "parental" or donor
jars chosen for selection in the previous generation. This "ecosystem
heritability" then quantifies what percentage of the variation in ecosystem function
is due to variation in the microbiome [@goodnight2000; @blouin2015].

We quantified how much of the variation in methane oxidation rate was due to
variation in the microbiome by regressing the mean methane oxidation rates of
...
`r myround((coefficients(herit_model)[2] + coefficients(herit_model)[4])*100,
1)`\% (SE = `r myround(tidy(herit_model)$std.error[4] * 100, 1)`\%) for the
Positive selection treatment. This estimate is comparable to the heritability
of carbon dioxide emissions from water [@blouin2015]. Notably, we observed a
negative heritability in the Neutral treatment 
(`r myround(coefficients(herit_model)[2]*100, 1)`\%, 
SE = `r myround(tidy(herit_model)$std.error[2] * 100, 1)`\%). Negative heritability
estimates are not uncommon in quantitative genetics [@steinsaltz2020]. This can
occur for random selection controls in artificial selection experiments and may
indicate that methane oxidation rate is negatively correlated with other traits
associated with persistence in jars. 
The heritability we observed in the Positive treatment indicates that
roughly half of the variation in methane oxidation rate is due to variation in
the microbiome in our experimental system.

Given that variation in the microbiome is driving the observed variation in
methane oxidation rate, we next wanted to address what aspects of the microbiome, in
terms of alpha diversity, beta diversity, or the relative abundance of taxa,
explain this contribution of the
...
ecosystem-scale methane oxidation rates can be altered by non-methanotrophs, perhaps
through ecological interactions with methanotrophic species. 
...
isolation, this seems like an unlikely explanation [@whittenbury1970;
@hirayama2014; @kim2018]. Future work could analyze an artificial ecosystem
selection experiment using genome-resolved metagenomics to try to detect
strain-level changes in the constituent taxa. However, reconstructing genomes
from in situ soil samples has been difficult to achieve and may require
enrichment techniques, particularly for methanotrophs which are a relatively
rare component of the soil microbiome [@delmont2015].  If these challenges
could be overcome for soil metagenomics, we could begin to evaluate whether
ecosystem selection results in strain-level evolution or enriches for certain
metabolic pathways within the microbiome.
...
`r figures("herit", paste0("Regression of mid-recipient methane flux on mid-donor methane flux ($-k$ with units day^-1^). Donor is the mean oxidation rate of the jars selected to inoculate the next passage. Recipient is the mean oxidation rate of the twelve jars in the next passage. The orange lines and points are the Positive treatment and the gray lines and points are the Neutral treatment. Slope of the regression line estimates the proportion of variation in methane oxidation rate due to variation in the microbiome. For the Positive treatment, the variance explained by the microbiome was ", myround((coefficients(herit_model)[2] + coefficients(herit_model)[4])*100, 1), "% (SE = ", myround(tidy(herit_model)$std.error[4] * 100, 1), "%) and for the Neutral treatment the variance explained by the microbiome was ", myround(coefficients(herit_model)[2]*100, 1), "% (SE = ", myround(tidy(herit_model)$std.error[2] * 100, 1), "%)."))`
