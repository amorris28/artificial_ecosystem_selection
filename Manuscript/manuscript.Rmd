---
title: "Artificial ecosystem selection on soil methane oxidation reveals simple/complex mapping between microbial community structure and ecosystem function"
...
figures("herit", "Correlation between mid-parent CH~4~ flux and mid-offspring CH~4~ flux.")
...
```{r sel_fig, fig.width=5, fig.height=4, fig.cap=figures("selection")}
fluxes %>% 
...
fit <- lm(offspring ~ parental, data = heritability)
her_sum <- tidy(summary(fit))
her_aov <- tidy(anova(fit))
```

Methane oxidation rate was heritable between consecutive generations (`r figures("herit", display="cite")`). Specifically, the narrow-sense heritability ($h^2$), calculated as the slope of the mid-parent and mid-offspring, was `r myround(her_sum[2, 2], 2)`.
