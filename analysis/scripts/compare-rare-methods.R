...
analysis %>%
  select(sample, n_seqs, sobs, chao, rare, est) %>%
...
ra_aes_df <- 
    rand_aes_df/rowSums(rand_aes_df)
ra_aes_dist <- vegdist(ra_aes_df, method="euclidean")
...
inner_join(norare_aes_dtbl, rare_aes_dtbl, by=c("sample", "name")) %>%
  ...
  inner_join(., zclr_aes_dtbl, by=c("sample", "name")) %>%
    inner_join(., ra_aes_dtbl, by=c("sample", "name")) %>%
  ...
  select(-n_seqs.x, -n_seqs.y) %>%
  pivot_longer(cols=c(norare, rare, rclr, zclr, ra), names_to="method", values_to="dist") %>%  
