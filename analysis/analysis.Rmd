...
ggsave(here(fig_dir, 'fig2.png'), p_response_cl, width = 88, height = 65, units = 'mm')
...
ggsave(here(fig_dir, 'fig3.png'), p_beta_cl,
       height = 60,
       width = 180,
       units = 'mm')
...
aes_family_G5 <-
  aes_G5 %>% 
  select(asv, sample, treat, passage, count, Family) %>%
...
# ps <- phyloseq(sample_data = sample_data(sample_data), 
#                otu_table = otu_table(otu_table,
#                taxa_are_rows = TRUE))
...
ggsave(here(fig_dir, 'fig4.png'), p_log10_cl, 
       width = 100, 
       height = 88,
       units = 'mm')
...
# Functional gene analysis with Picrust

```{r, eval=FALSE}

metacyc_pathways <- read_tsv(here(der_dir, 'picrust2_out_pipeline/pathways_out/path_abun_unstrat.tsv.gz'))
metacyc_pathways <- metacyc_pathways[,1:49]
metacyc_pathways <- as.data.frame(metacyc_pathways)
rownames(metacyc_pathways) <- metacyc_pathways[, 1]
metacyc_pathways <- metacyc_pathways[, -1]
metadata <- data.frame(conds = substr(colnames(metacyc_pathways), 3, 3))
rownames(metadata) = colnames(metacyc_pathways)
sample_data = sample_data(metadata)
otu_table = otu_table(metacyc_pathways, taxa_are_rows = TRUE)
metacyc_ps <- phyloseq(otu_table, sample_data)
taxa_names(metacyc_ps)
# Save/Load output so you don't have to rerun ANCOM
if(file.exists(here(der_dir, "ancom_out_metacyc.rds"))) {
  out <- 
    readRDS(here(der_dir, "ancom_out_metacyc.rds"))
} else {
  # Run Ancom
  out = ancom(data = metacyc_ps,  
              p_adj_method = "BH",
              prv_cut = 0.1, 
              lib_cut = 0, 
              main_var = "conds",
              ...
              n_cl = 1)  
  out %>% 
    saveRDS(here(der_dir, "ancom_out_metacyc.rds"))
}
...
ancom_taxa <- as.character(df_fig_w[df_fig_w$detected_0.7, ]$taxon)
metacyc_pathways_descrip <- read_tsv(here(der_dir, 'picrust2_out_pipeline/pathways_out/path_abun_unstrat_descrip.tsv.gz')) %>% 
    select(pathway, description)
metacyc_pathways_descrip[metacyc_pathways_descrip$pathway %in% ancom_taxa,] ->
    sig_pathways
```
